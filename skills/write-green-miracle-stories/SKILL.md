...
description: 以綠色奇蹟核定知識母稿、Notion 授權資料及公開來源，撰寫可引用的年度觀察、Wiki、紀實文章、官網文案、簡報、行銷與教育訓練素材。也用於規劃 MkDocs 導覽與內頁目錄短標題、排除目錄中重複的頁面標題、減少同頁重複提示詞，或避免把內部禁止語句與治理要求輸出到核准的對外文章。
---
...
- 內頁目錄從 H2 開始，頁面 H1 只在正文顯示一次，不在側邊目錄重複。MkDocs `toc_depth` 使用 `"2-2"`。
- 同一頁的提示框、段落開頭與小標先檢查重複用詞。相同標籤原則上只出現一次；多個提示框改用能說明該段作用的專屬短語，或合併沒有新增資訊的段落。
